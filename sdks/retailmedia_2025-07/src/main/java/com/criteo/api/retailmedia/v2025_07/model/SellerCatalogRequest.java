...
import com.criteo.api.retailmedia.v2025_07.model.SellerIdentifier;
...
public class SellerCatalogRequest {
  ...
  private List<SellerIdentifier> sellers = new ArrayList<>();
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SellerCatalogRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SellerCatalogRequest` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SellerCatalogRequest.openapiRequiredFields) {
